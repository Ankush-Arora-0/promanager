...
const allTask = (req, res) => {
    ...
    task.find(req.body).populate('employeeId').populate('subcategoryId').populate("projectId").sort({createdAt: -1}).exec()
        .then((result) => {
            res.send({
                success: true,
                status: 200,
...
const singleTask = (req, res) => {
    ...
    if (!!validation) {
        ...
        task.findOne({ _id: req.body._id }).populate('subcategoryId').populate("projectId").populate("employeeId").exec()
            .then((result) => {
                if (result == null) {
                    res.send({
                        success: false,
